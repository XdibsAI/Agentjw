...
def analyze_ports():
    ...
    # Check actual listening ports — `ss -ltnp` often requires root to map
    # ports to process names correctly, giving false/misleading results
    # (e.g. picking up an unrelated process on 8080). Instead, inspect the
    # cmdline of running uvicorn/api_server processes directly.
    actual_ports = set()
    _, ps_out, _ = run_cmd("ps aux | grep -E 'uvicorn|api_server' | grep -v grep")
    for line in ps_out.splitlines():
        m = re.search(r'--port[=\s]+(\d{4,5})', line)
        if m:
            actual_ports.add(int(m.group(1)))

    # Fallback to ss if no process-based match found (best-effort)
    if not actual_ports:
        _, listening, _ = run_cmd("ss -ltn")
