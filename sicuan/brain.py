...
from sicuan.core.error_fix_map import get_error_fixer
...
PENTING: Jika user mengirim URL (https://...) dan minta analisis/integrasi:
- action = analyze_url
- Target = URL yang dikirim
- JANGAN pilih analyze_project
- Berikan analisis spesifik tentang library/URL tersebut
- SETELAH memberikan analisis URL, TETAP fokus pada topik yang sama
- JANGAN tiba-tiba pindah ke project lain (flask_todo_api)
- Jawab langsung dengan analisis + rekomendasi
- Akhiri dengan call to action yang relevan
...
"""
    ...
    def fetch_url_content(self, url: str) -> str:
        """Fetch content from URL dan berikan analisis"""
        try:
            import requests
            import re
            ...
            if response.status_code == 200:
                if "github.com" in url:
                    ...
                    # Extract repo name
                    repo_name = url.replace("https://github.com/", "").split("/")[0] + "/" + url.replace("https://github.com/", "").split("/")[1] if "/" in url.replace("https://github.com/", "") else url
...
"""
    ...
    def diagnose_error(self, error_message: str) -> dict:
        ...
        return fixer.diagnose(error_message)

    def auto_fix_error(self, error_message: str) -> dict:
        """Auto fix error: diagnose → fix → validate"""
        # 1. Diagnose
        diagnosis = self.diagnose_error(error_message)
        
        if diagnosis.get("action") != "modify_logic":
            return {
                "status": "skipped",
                "message": f"Tidak bisa auto-fix. Action: {diagnosis.get('action')}",
                "diagnosis": diagnosis
            }
        
        # 2. Execute modify_logic
        result = self.execute_action(
            action="modify_logic",
            target=diagnosis.get("file", "strategy.py"),
            user_request=f"Perbaiki {diagnosis.get('fix')}",
            session_id="auto_fix"
        )
    ...
    def fix_and_report(self, error_message: str) -> str:
        """Fix error dan laporkan hasil"""
        ...
        # 2. Auto-fix
        fix_result = self.auto_fix_error(error_message)
    ...
    def restart_bot(self) -> bool:
        """Restart bot dan cek apakah berhasil"""
        try:
            import subprocess
            import time
            from pathlib import Path
            ...
            # Start bot
            bot_path = Path("projects/godmeme_bot/main.py")
    ...
    def start_bot_with_error(self) -> dict:
        """Start bot dan tangkap error jika gagal"""
        import subprocess
        import time
        from pathlib import Path
        ...
        # Gunakan error_capture untuk traceback lengkap
        ...
        if capture_result["success"]:
            ...
            result["error"] = capture_result.get("traceback", capture_result.get("error_message", "Unknown error"))
            result["traceback"] = capture_result.get("traceback", "")
            result["output"] = capture_result.get("stderr", "")
            return result
        ...
        try:
            ...
            # Start bot dan tangkap output
            # Gunakan path absolut
            bot_path = Path("/home/dibs/agentjw/projects/godmeme_bot/main.py")
            if not bot_path.exists():
                result["error"] = f"Bot not found: {bot_path}"
                # === VALIDASI ===
                validation = validator.validate({"patch": result})
                if validation["status"] == "verified":
                    result += "\n\n✅ **Validation**: " + validation["reason"]
            ...
            process = subprocess.Popen(
                ["python3", str(bot_path)],
                cwd="/home/dibs/agentjw/projects/godmeme_bot",
                stdout=subprocess.PIPE,
                stderr=subprocess.PIPE
            )
            
            # Tunggu sebentar dan cek
            time.sleep(3)
            
            # Cek apakah masih running
            if process.poll() is not None:
                # Bot exited with error
    ...
    def get_repair_strategy(self, error: dict) -> str:
        """Dapatkan strategi perbaikan berikutnya"""
        memory = get_repair_memory()
        return memory.get_next_strategy(error)
