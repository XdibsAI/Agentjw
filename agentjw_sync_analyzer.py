...
def analyze_brain_intent():
    ...
    if brain_path.exists():
        ...
        for m in re.finditer(r'["\']([a-z_]+)["\'].*?intent|intent.*?["\']([a-z_]+)["\']', content):
            intent = m.group(1) or m.group(2)
            if intent and len(intent) > 3:
                brain_intents.add(intent)
        # Also look for decide() function return values
        for m in re.finditer(r'return\s+["\']([a-z_]+)["\']', content):
    ...
    if brain_path.exists():
        ...
        if "def decide" in content:
            ...
            for intent in orch_intents_defined:
                if intent not in content and intent != "chat":
...
def analyze_tools():
    ...
    for tf in set(all_tool_files):
        ...
        for m in re.finditer(r'^(\w+)\s*=\s*\w+\(', content, re.MULTILINE):
            inst = m.group(1)
            if not inst.startswith("_") and inst[0].islower():
